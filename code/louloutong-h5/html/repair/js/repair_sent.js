    ...
    success:function(data){
        ...
        if(data.code === 0){
            $.each(data.data,function(index,val){
                html += '<li> <a href="'+ headJumps(val.id) +'"> <img class="avatar" src="'+ server_uel_user_img + val.photo +'" onerror="defaultPA(this)"> </a> <a data-id="'+ val.id +'" class="orders" href="javascript:"> <div class="information"> <div class="name"><span class="na">'+ val.name +'</span><span class="position">'+ val.duty[0] +'</span></div> <time>上班时间：08:00-23:00</time> <div class="picked-up">已有'+ val.count +'单</div> </div> </a> </li>'
            })
        }
