...
$(document).ready(function(){
    // 请假列表	html/duty/leave.html	100130002
    // 请假审批		1001300020001
    isShow('','1001300020001');
    if(isEdit){
        $(".con-oper").show();
    }else{
        $(".con-oper").hide();
...
function label(page,size,keyword){
    ...
    $.ajax({
        type:'post',
        url:server_url + '/web/webDutyCtr/findDutyLeaveList',
            ...
            if(data.code === 200){
                if(data.data.content.length!=0){
                    ...
                    $.each(data.data.content,function(index,val){
                        ...
                        listData += '<tr> <td><input data-bur="'+ bur +'" data-id="'+ val.id +'" type="checkbox"><i></i></td> <td>'+ (index + 1) +'</td> <td>'+ val.name +'</td> <td>'+ val.leaveContent +'</td> <td>'+ (new Date(val.createTime)).toLocaleString() +'</td> <td>'+ (new Date(val.startTime)).toLocaleString() +'</td> <td>'+ (new Date(val.endTime)).toLocaleString() +'</td> <td>'+ tdCheck(val.duration) +'</td> <td>'+ status +'</td> <td class="coazhuo">'+ statusBtn +'</td> </tr>'
...
function single(_this,bur){
    ...
    if(bur=='yes'){
        confirmMsg("是否同意请假？",function(dialog){
            dialog.close();
            approval(data,true);
        })
    }else{
        confirmMsg("是否不同意请假？",function(dialog){
            dialog.close();
            approval(data,true);
        })
    }
