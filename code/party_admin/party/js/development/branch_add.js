            ...
            success:function(data){
                ...
                if(data.code === 0){
                    ...
                    $.each(data.data.items,function(index,item){
                        ...
                        html+='<tr><td><input name="user" data-firmId="'+item.firmId+'" data-date="'+item.joinPartyDate+'" data-id="'+item.id+'" type="radio"></td><td>'+item.userName+'</td><td>'+item.name+'</td><td>'+item.phone+'</td><td data-type="'+item.type+'">'+stage+'</td><td>'+item.organName+'</td></tr>';
                    });
...
function confirm(){
    ...
    if(userlist_tab.length!=0) {
        ...
        firmId= userlist_tab.attr("data-firmId");
        var joinPartyDate=userlist_tab.attr("data-date");
        var organName=$(userlist_tab.parents("tr").find("td")[5]).text();
        ...
        $("#joinPartyDate").text(joinPartyDate);
...
//新增的确定
function editAjax(_this){
    var status=$(_this).text()=="发布"?1:0;
    var data={
        status:status,
        userId:dataID,
        type:3,
    ...
    if($("#newForm").valid()){
        $.ajax({
            type:'POST',
            url: DMBServer_url + '/web/api/article/add.json',
            data:data,
            dataType:'json',
            success:function(data){
                console.log(data);
