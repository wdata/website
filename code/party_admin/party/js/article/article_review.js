        ...
        success:function(data){
            ...
            if(data.code === 0){
                ...
                $.each(data.data.items,function(index,val){
                    ...
                    listData += '<tr> <td><input onclick="checkDown(this)" type="checkbox"  data-id="'+ val.id +'"></td> <td>'+ (index + 1 + page * 10) +'</td> <td>'+ val.title +'</td> <td>'+ type +'</td> <td>'+ val.author +'</td> <td>'+ val.createTime +'</td> <td>'+ val.companyName +'</td> <td> <a onclick="yes(this)" class="oper-status  opened">通过</a> <a onclick="no(this)"  class="oper-status  closed">不通过</a> </td> <td class="caozhuo"> <button class="btn btn-info btn-dialog btn-dialog-tab" onclick="view(this)" href="party/article/article_review/article_view.html">查看</button> <button class="btn btn-info btn-dialog btn-dialog-tab" href="party/article/article_review/article_review_edit.html" onclick="modifyClick(this)" >修改</button> <a onclick="deteleAlone(this)" >删除</a> </td> </tr>';
                });
...
function deteleAlone(wo){
    var self = wo;
    confirmMsg("是否删除新闻",function(dialog){
...
function no(self){
    //出现弹出框，输入原因
    $(".modal").addClass('show').addClass("in");
    session("no_Id",$(self).parents("tr").find("input[type=checkbox]").attr("data-id"));
}
//-------------------------------不通过       多选-------------------------
function noAll(){
    var dataID = $('#lebel_tab tbody input[type=checkbox]:checked');   //单选按钮
...
//------------------取消----隐藏弹出框
function picEditNo(){
    $(".modal").removeClass('show').removeClass("in");
}
//------------------确定----上传
function picEditYes(){
    var id = sessionStorage.getItem("no_Id");
    var reason = $("#reason").val();
    onOFF(id,-1,reason);
}

function onOFF(id,status,reason){
    $.ajax({
        type:'post',
        url: DMBServer_url + '/web/api/news/updateStatus.json',
        data:{
            ids:id,    //台账模板ID，数组
        ...
        success:function(data){
            if(data.code === 0){
                switch (status){
                    case 1:
                        //通过成功
                        returnMessage(1,'通过成功！');
                        break;
                    case -1:
                        //不通过成功
                        returnMessage(1,'不通过成功！');
                        $(".modal").removeClass('show').removeClass("in");
                        break;
                }
                label(paGe,pageNum);
